...
import com.usdk.apiservice.aidl.extprinter.UExtPrinter;
...
import com.usdk.apiservice.aidl.indicatorlight.UIndicatorLight;
...
import com.usdk.apiservice.aidl.system.security.UVendorCertificate;
...
public final class DeviceHelper implements ServiceConnection {
	...
	public UVendorCertificate getVendorCertificate() throws IllegalStateException {
		IBinder iBinder = new IBinderCreator(){
			@Override
			IBinder create() throws RemoteException {
				return getSystem().getVendorCertificate();
			}
		}.start();
		return UVendorCertificate.Stub.asInterface(iBinder);
	}
    ...
    public UExtPrinter getExtPrinter() {
		IBinder iBinder = new IBinderCreator(){
			@Override
			IBinder create() throws RemoteException {
				return deviceService.getModule(ModuleName.EXTERNAL_PRINTER, new Bundle());
			}
		}.start();
		return UExtPrinter.Stub.asInterface(iBinder);
    }

	public UIndicatorLight getIndicatorLight() {
		IBinder iBinder = new IBinderCreator(){
			@Override
			IBinder create() throws RemoteException {
				return deviceService.getModule(ModuleName.INDICATOR_LIGHT, new Bundle());
			}
		}.start();
		return UIndicatorLight.Stub.asInterface(iBinder);
	}
